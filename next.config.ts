...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    return [
      // Canonical host: apex (sergioluque.com), not www. Without this
      // rule both apex and www serve the SAME content with HTTP 200,
      // which Google indexes as duplicate content and the canonical
      // signal in the page's <head> is the only thing nudging it back
      // toward the apex. A 308 at the edge is unambiguous. Matched on
      // `host` header — Cloudflare Pages forwards Host correctly.
      {
        source: '/:path*',
        has: [{ type: 'host', value: 'www.sergioluque.com' }],
        destination: 'https://sergioluque.com/:path*',
        permanent: true,
      },
      // Public-route renames. The earlier public/_redirects file was
      // kept around but Cloudflare Pages with @cloudflare/next-on-pages
      // routes everything through the generated _worker.js, which never
      // consults the static _redirects file — so those rules silently
      // 404'd in production. next.config.ts redirects() are baked into
      // the worker bundle and DO fire. permanent: true emits a 308
